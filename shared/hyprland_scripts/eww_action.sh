...
case "$action" in
    ...
    apps)
        ...
        rofi -show drun -theme-str \
            'window {location: north west; anchor: north west; x-offset: 12px; y-offset: 50px;}' &
        ;;
    ...
    windows)
        ...
        rofi -show window \
            ...
            -theme-str 'window {location: north west; anchor: north west; x-offset: 12px; y-offset: 50px; width: 50%;}' &
        ;;
    ...
    sync_theme)
        current_wp="$(swww query 2>/dev/null | awk '/currently displaying/ {print $NF}')"
