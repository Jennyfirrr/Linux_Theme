...
# ╰──────────────────────────────────────────────╯
# Palette: peach(173) pink(173,173) mauve(180) lavender(139,141)
...
function gradient_path() {
  local raw="$1"
  local -a colors=(180 173 138 180 173 138 180)
  local -a segments
  local result="" idx=0

  local prefix=""
  if [[ "$raw" = "~/"* ]]; then
    prefix="%{%F{180}%} ~%{%F{173}%}/%{%f%}"
    raw="${raw#\~/}"
  elif [[ "$raw" = "~" ]]; then
    echo "%{%F{180}%} ~%{%f%}"
    return
...
ZSH_THEME_GIT_PROMPT_PREFIX="%{%F{173}%} "
...
function caramel_precmd() {
  ...
  if [[ $exit_code -eq 0 ]]; then
    PROMPT_INDICATOR="%{%F{180}%}❯%{%f%}"
  else
    PROMPT_INDICATOR="%{%F{167}%}❯ %{%F{180}%}${exit_code}%{%f%}"
  fi
  ...
  if (( _caramel_timer_start > 0 )); then
    ...
    if (( elapsed_int >= CARAMEL_CMD_THRESHOLD )); then
      ELAPSED_PROMPT=" %{%F{173}%} $(fmt_elapsed $elapsed_int)%{%f%}"
    fi
    _caramel_timer_start=0
  fi
  ...
  if [[ -n "$CONDA_DEFAULT_ENV" && "$CONDA_DEFAULT_ENV" != "base" ]]; then
    VENV_PROMPT=" %{%F{138}%} %{%F{180}%}$(basename "$CONDA_DEFAULT_ENV")%{%f%}"
  elif [[ -n "$VIRTUAL_ENV" ]]; then
    VENV_PROMPT=" %{%F{138}%} %{%F{180}%}$(basename "$VIRTUAL_ENV")%{%f%}"
  fi

  # ── SSH indicator ──
  SSH_PROMPT=""
  if [[ -n "$SSH_CONNECTION" ]]; then
    SSH_PROMPT="%{%F{173}%} %m%{%f%} "
  fi
  ...
  if (( njobs > 0 )); then
    JOBS_PROMPT=" %{%F{173}%} ${njobs}%{%f%}"
  fi

  # ── Host + Path + Git ──
  CARAMEL_HOST="%{%F{173}%}%n%{%F{138}%}@%{%F{173}%}%m%{%f%}"
